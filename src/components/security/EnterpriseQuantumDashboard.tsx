...
export function EnterpriseQuantumDashboard() {
  ...
  const { simulateAttacks, logSecurityAttack } = useSecurityAttacks();
  const [testingAttack, setTestingAttack] = useState(false);
  ...
  const handleSimulateAttack = async () => {
    setTestingAttack(true);
    try {
      await simulateAttacks();
      toast({
        title: "Quantum Attack Simulated",
        description: "Attack has been detected and logged. Check Attack Logs tab.",
      });
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message || "Failed to simulate attack",
        variant: "destructive",
      });
    } finally {
      setTestingAttack(false);
    }
  };

  const handleLogRandomEvent = async () => {
    setTestingAttack(true);
    try {
      const eventTypes = ['privilege_escalation', 'unauthorized_access', 'data_breach_attempt'];
      ...
      toast({
        ...
        description: "Random security event has been logged successfully.",
      });
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message || "Failed to log event",
        variant: "destructive",
      });
    } finally {
      setTestingAttack(false);
    ...
    <div className="space-y-6">
      ...
      <Card className="border-amber-500/50 bg-amber-50/5">
        <CardHeader>
          ...
          <CardDescription>
            Test quantum security features and simulate attack scenarios
          </CardDescription>
        </CardHeader>
        <CardContent className="flex gap-3">
          <Button
            onClick={handleSimulateAttack}
            disabled={testingAttack}
            variant="outline"
            className="flex-1"
          >
