...
export function useSecurityAttacks() {
  ...
  const logSecurityAttack = async (
    ...
    try {
      ...
      // Also log to quantum_attack_logs for trigger-based audit logging
      const { error: quantumError } = await supabase
        ...
        .insert({
          ...
          severity: severity,
          is_blocked: true,
          detection_method: quantumProtected ? 'quantum_shield' : 'standard_firewall',
          attack_signature: `${attackType}_${Date.now()}`,
          metadata: attackData
        });
  ...
  // Simulate realistic attack patterns with comprehensive logging
  const simulateAttacks = async () => {
    ...
    await logSecurityAttack(
      ...
      randomAttack.severity,
      {
        ...
        attack_vector: randomAttack.type,
